...
INTERVAL = "1m" INTRADAY_PERIOD = "1d" last_alert = {} last_update_id = None last_scan_summary = "No scan yet" last_top3 = []

==============================================

def send(msg): try: requests.post(TG_URL, json={"chat_id": CHAT_ID, "text": msg}, timeout=15) except Exception as e: print("Telegram send error:", e)

==============================================

def get_updates(offset=None): params = {"timeout": 5} if offset is not None: params["offset"] = offset try: r = requests.get(UPDATES_URL, params=params, timeout=10) r.raise_for_status() return r.json().get("result", []) except Exception as e: print("Telegram update error:", e) return []

==============================================

def get_data(symbol, interval=INTERVAL, period=INTRADAY_PERIOD): try: df = yf.download(symbol, interval=interval, period=period, progress=False, auto_adjust=False) return df except Exception: return None

==============================================

def get_spy_trend(): spy = get_data("SPY") if spy is None or spy.empty or len(spy) < 25: return "NEUTRAL"
...
def bad_day_filter(spy_trend, df): if spy_trend == "NEUTRAL": return True, "SPY neutral"
...
def build_trade(symbol, trend): df = get_data(symbol) if df is None or df.empty or len(df) < 30: return None
...
def build_top_message(top3): if not top3: return "📋 TOP 3 TODAY\nNo strong gappers found"
...
def scan(): global last_scan_summary, last_top3
...
def build_status(): now = datetime.now(SG_TZ).strftime("%Y-%m-%d %H:%M:%S") in_session = (21 <= datetime.now(SG_TZ).hour or datetime.now(SG_TZ).hour < 2) return ( "📊 BOT STATUS\n" f"Time: {now}\n" f"Session Active: {'YES' if in_session else 'NO'}\n" f"Universe Size: {len(BASE)}\n" f"Top 3 Ready: {', '.join([x[0] for x in last_top3]) if last_top3 else 'Not yet'}\n" f"Last Scan:\n{last_scan_summary}" )

==============================================

def process_commands(): global last_update_id updates = get_updates(None if last_update_id is None else last_update_id + 1)
...
def main(): send("🚀 ELITE+ FINAL BOT STARTED") print("Bot started...")
